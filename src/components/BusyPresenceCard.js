...
const CalendarCard = ({title, schedule, type}) => {
     return (
        schedule.length
        ? <Card title={`${title} Schedule`} extra={type}>
            <Timeline>
                {schedule.map((item, index) => (
                    <Timeline.Item color={type === 'BUSY_TIME' ? 'red' : 'blue'} key={index}>{item.start} - {item.end}</Timeline.Item>
                ))}
            </Timeline>
        </Card>
        : ''
...
class BusyPresenceCard extends Component {
    ...
    render() {
        ...
        return (
            <Row>
                <Col {...layout}>
                    <CalendarCard title={"Google Calendar"} schedule={googleCalendar} type="FREE TIME"></CalendarCard>
                </Col>
                <Col {...layout}>
                    <CalendarCard title={"office365 Calendar"} schedule={office365Calendar} type="BUSY TIME"></CalendarCard>
                </Col>
            </Row>
        )
    }
